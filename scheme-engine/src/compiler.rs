...
pub fn compile(env: Handle<Env>, expr: &Expr) -> Result<Program> {
    ...
    let (env, proc) = compiler.take_procedure()?;
    ...
    let closure = Handle::new(Closure::new(Rc::new(proc)));

    Ok(Program { env, closure })
}

/// An executable Scheme program.
pub struct Program {
    /// Keeps a shared reference to the environment used
    /// to compile the closure.
    ///
    /// This is to keep the environment from being dropped
    /// while the closure is alive.
    ///
    /// Procedures keep weak references to their lexical environments
    /// to avoid a circular reference causing a memory leak.
    env: Handle<Env>,
    /// The executable code is packaged in a top-level closure, acting
    /// as the entry point for the program.
    closure: Handle<Closure>,
}

impl Program {
    /// The entrypoint for the program. This can be passed to [`scheme_engine::eval()`]
    /// to execute the top-level code.
    #[inline(always)]
    pub fn closure(&self) -> &Handle<Closure> {
        &self.closure
    }
}
...
impl Compiler {
    ...
    fn compile_form_pair(&mut self, pair: &Handle<Pair>) -> Result<()> {
        if self.compile_special_form_pair(pair)? {
            Ok(())
        } else {
            // The default s-expression form is a procedure call.
            self.compile_call_pair(pair)
        }
    }

    fn compile_call_pair(&mut self, pair: &Handle<Pair>) -> Result<()> {
        todo!()
    }

    fn compile_special_form_pair(&mut self, pair: &Handle<Pair>) -> Result<bool> {
        if let (Expr::Ident(operator), rest) = pair.borrow().split_first() {
    ...
    fn compile_if_form_pair(&mut self, expr: &Expr) -> Result<()> {
        todo!("if form pair")
    }
